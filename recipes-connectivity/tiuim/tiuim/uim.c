...
static char dev_name_arg[48];
static long baud_rate_arg;
static int flow_ctrl_arg;
...
int st_uart_config(unsigned char install)
{
	...
	if (install == '1') {
		...
		dev_fd = open(dev_name_arg, O_RDWR);
		if (dev_fd < 0) {
		...
		if (baud_rate_arg != 115200) {
			...
			UIM_VER("Speed changing to %ld, %d", baud_rate_arg, flow_ctrl_arg);
			/* Set the actual custom baud rate at the host side */
			if (set_custom_baud_rate(dev_fd, baud_rate_arg, flow_ctrl_arg) < 0) {
...
void usage(void)
{
	...
	UIM_ERR("Usage: uim [ -f <path-to-sysfs-kim> ] [ -b <bd address XX:XX:XX:XX:XX:XX> ] [ -sdc ]"
            " options:"
            "  -s speed (baudrate)"
            "  -d the uart device"
            "  -c if flow control should be used");
}
...
int main(int argc, char *argv[])
{
	...
	while ((opt = getopt (argc, argv, "f:b:s:d:c")) != -1) {
		switch (opt) {
			case 'f':
				arg_kim_path = optarg;
				break;
			case 'b':
				...
				break;
            case 's':
                baud_rate_arg = atoi(optarg);
                break;
            case 'd':
                strncpy(dev_name_arg, optarg, sizeof(dev_name_arg));
                break;
            case 'c':
                flow_ctrl_arg = 1;
                break;
